...
def main(domain: str, num_train_data: int, layer_type: str, seed: int):
    ...
    logging_file = f'log_{timestamp}.log'
    ...
    logging.info(f"{value_model.local_bidder_ids=}, {value_model.regional_bidder_ids=}, {value_model.national_bidder_ids=}")
    ...
    ref_bidder = list(NN_parameters.keys())[0]  # set all architectures to the local configration
    for bidder in NN_parameters.keys():
        NN_parameters[bidder] = NN_parameters[ref_bidder]
    ...
    kwargs = {
        ...
        'local_scaling_factor': None if layer_type == 'PlainNN' else None,
...
if __name__ == '__main__':
    ...
    parser.add_argument('--seed', type=int, default=10000, help='SATS auction instance seed.')
