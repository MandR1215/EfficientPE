...
def mlca_mechanism(SATS_auction_instance_seed=None, 
                   ...
                   Qinit=None, Qmax=None, Qround=None,
                   NN_parameters=None, MIP_parameters=None, scaler=None,
                   init_bids_and_fitted_scaler=[None, None], return_allocation=False, return_payments=False,
                   calc_efficiency_per_iteration=False, isLegacy=False, 
    ...
    if configdict is not None:
        ...
        value_model = configdict['value_model']
    ...
    if SATS_domain_name == 'GSVM':
        ...
        logging.warning('#########################\n')
    if SATS_domain_name == 'MRVM': # create SATS auction instance
        SATS_auction_instance = PySats.getInstance().create_mrvm(
            seed=SATS_auction_instance_seed,
            number_of_local_bidders=len(value_model.local_bidder_ids),
            number_of_regional_bidders=len(value_model.regional_bidder_ids),
            number_of_national_bidders=len(value_model.national_bidder_ids))
    ...
    OUTPUT['Global Efficient Allocation'] = E.SATS_auction_instance_allocation
    OUTPUT['Global Efficient SCW'] = E.SATS_auction_instance_scw
    OUTPUT['Allocation'] = E.mlca_allocation
    with open(logging_file.replace(".log", ".pkl"), "wb") as f:
        import pickle
...
# %%
logging.info('FUNCTION MLCA imported')
