...
problem_instance_info = {
    ...
    'MRVM': instance_info(bidder_types=['Local', 'Regional', 'National'], Qmax=20, Qround=1, Qinit=1),
...
def _create_LSVM(number_of_items=18, 
                 local_bidder_ids=[], 
                 regional_bidder_ids=list(range(1, 6)),
                 national_bidder_ids=[0], 
                 scaler=[None], 
                 highFrequency_bidder_ids=[]):
    return ValueModel(name='LSVM', 
                      number_of_items=number_of_items, 
                      local_bidder_ids=local_bidder_ids, 
                      regional_bidder_ids=regional_bidder_ids, 
                      national_bidder_ids=national_bidder_ids, 
                      scaler=scaler, 
                      highFrequency_bidder_ids=highFrequency_bidder_ids)

def _create_GSVM(number_of_items=18, 
                 local_bidder_ids=[], 
                 regional_bidder_ids=list(range(0, 6)),
                 national_bidder_ids=[6], 
                 scaler=[None], 
                 highFrequency_bidder_ids=[]):
    return ValueModel(name='GSVM', 
                      number_of_items=number_of_items, 
                      local_bidder_ids=local_bidder_ids, 
                      regional_bidder_ids=regional_bidder_ids, 
                      national_bidder_ids=national_bidder_ids, 
...
def create_value_model(value_model:str, **kwargs) -> ValueModel:
    model_function_map = {
        'LSVM': _create_LSVM,
        'GSVM': _create_GSVM,
        'MRVM': _create_MRVM,
    ...
    if value_model not in model_function_map:
        raise NotImplementedError('Unknown value model: {}'.format(value_model))

    return model_function_map[value_model](**kwargs)
