...
class MLCA_Economies:
    ...
    def __init__(self, SATS_auction_instance, SATS_auction_instance_seed, Qinit, Qmax, Qround, scaler,
        ...
        _, self.__default_test_data = self.__generate_default_test_data(num_points=300)
        self.test_results = OrderedDict()
        
    def __float_to_digit(self, a, num_digit):
        binary_str = ''
    ...
    def __generate_default_test_data(self, num_points:int=100):
        num_digit = self.M
        arr = np.linspace(0, 1, num_points, endpoint=False)
        ret = []

        for a in arr:
            ret.append(self.__float_to_digit(a, num_digit))
        return arr, np.asarray(ret)

    def __create_test_data(self) -> np.ndarray:
        binary_vec_to_str = lambda x: ''.join(list(map(str, x)))

        all_elicited_bids = set()
        for (elicited_bids,_) in self.elicited_bids.values():
            for bid in elicited_bids:
                str_of_bid = binary_vec_to_str(bid)
                all_elicited_bids.add(str_of_bid)
        
        removed_test_data = []

        for data in self.__default_test_data:
            str_of_data = binary_vec_to_str(data)
            if str_of_data not in all_elicited_bids:
                removed_test_data.append(data)

        removed_test_data = np.asarray(removed_test_data)

        return removed_test_data
    
    def test(self, economy_key):
        test_data = self.__create_test_data()
        
        results = {'economy_key': economy_key, 
                   'test_data': test_data, 
                   'result': dict()}
        models = self.NN_models[economy_key]
        for bidder_id, (key, model) in zip(self.bidder_ids, models.items()):
            values = self.value_queries(bidder_id=bidder_id, bundles=test_data)
            (preds, targets), metric = model.evaluate(test_data, values)
            result = {'preds': preds, 'targets': targets, 'loss': metric}
            results['result'][key] = result
        
        self.test_results[self.mlca_iteration] = results
    ...
    def calculate_efficiency_per_iteration(self):
        ...
        allocation, objective = self.solve_WDP(self.elicited_bids, verbose=1, timelimit=200)
    ...
    def solve_SATS_auction_instance(self):
        self.SATS_auction_instance_allocation, self.SATS_auction_instance_scw = self.SATS_auction_instance.get_efficient_allocation(timelimit=200)

    def exp_random(self, num_exp:int=5):
        from tqdm import tqdm
        import pandas as pd
        def item_index_list_to_bundle(n_item:int, item_index_list:list):
            ret = [0]*n_item
            for item in item_index_list:
                ret[item] = 1
            return ret
        def generate_random_allocation(n_bidder:int, n_item:int):
            alloc = {bidder: [] for bidder in range(n_bidder)}
            for item in range(n_item):
                alloc[random.choice(list(alloc.keys()))].append(item)
            bin_alloc = {bidder: item_index_list_to_bundle(n_item, a) for (bidder, a) in alloc.items()}
            return alloc, bin_alloc
        def calculate_efficiency_of_allocation(model, alloc:dict):
            sum_value = 0
            for bidder, a in alloc.items():
                sum_value += model.calculate_value(bidder, a)
            return sum_value / model.get_efficient_allocation()[1]
        dic_exp = dict()
        for exp in tqdm(range(1, num_exp+1)):
            l_exp = []
            for _ in tqdm(range(self.Qmax), desc=f'Exp {exp}'):
                _, bin_random_alloc = generate_random_allocation(len(self.SATS_auction_instance.get_bidder_ids()), 
                                                                 len(self.SATS_auction_instance.get_good_ids()))
                eff = calculate_efficiency_of_allocation(self.SATS_auction_instance, bin_random_alloc)
                l_exp.append(eff)
            dic_exp[f'exp{exp}'] = l_exp
    
        df_exp = pd.DataFrame(dic_exp)
        logging.info(df_exp.describe())
    ...
    def update_elicited_bids(self):
        for bidder in self.bidder_names:
            ...
            logging.info('Elicited Bids %s', bidder)
            for bundle, value in zip(self.elicited_bids[bidder][0], self.elicited_bids[bidder][1]):
                logging.info('bundle: ' + ''.join([str(a) for a in bundle]) + f' | {value=}')
    ...
    def next_queries(self, economy_key, active_bidder):
        if not self.economy_status[economy_key]:  # check if economy already has been calculated prior
            ...
            self.test(economy_key=economy_key)
    ...
    def estimation_step(self, economy_key):
        ...
        logging.info('-----------------------------------------------')
        from mlca_src.mlca_multitask import eval_multitask_nn, multitask_soft_nn_parameters, multitask_hard_nn_parameters
        logging.info(f"{multitask_soft_nn_parameters=}")
        models = eval_multitask_nn(bidders=self.economies_names[economy_key],
                                   fitted_scaler=self.fitted_scaler,
                                   local_scaling_factor=self.local_scaling_factor,
                                   NN_parameters=multitask_soft_nn_parameters,
    ...
    def solve_WDP(self, elicited_bids, verbose=0, timelimit:int=None):  # rem.: objective always rescaled to true values
        ...
        wdp = MLCA_WDP(elicited_bundle_value_pairs, timelimit=timelimit)
        ...
        if self.fitted_scaler is not None:
            if verbose == 1:
                ...
                logging.info('---------------------------------------------')
                logging.info('WDP objective scaled: %s:', objective)
                logging.info('WDP objective value scaled by: 1/%s', round(self.fitted_scaler.scale_[0], 8))
            objective = float(self.fitted_scaler.inverse_transform([[objective]]))
            if verbose == 1:
                logging.info('WDP objective orig: %s:', objective)
                logging.info('---------------------------------------------')
...
# %%
logging.info('Class MLCA_Economies imported')
