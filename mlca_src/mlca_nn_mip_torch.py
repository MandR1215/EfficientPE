...
class NN_MIP_TORCH:
    ...
    def __init__(self,
        ...
        if self.NN_type == 'UNN':
            ...
            self.upper_bounds_z = OrderedDict(
                list((bidder_name, 
                      {idx + 1: val for idx, val in enumerate(np.array([None] * layer.shape[0], 
                                                                       dtype=np.float32).reshape(-1, 1) 
                                                              for layer in self._get_model_layers(bidder_name, exc_layer_type=['output']))
                      }) 
            ...
            self.active = OrderedDict(
                list((bidder_name, 
                      {idx + 1: val for idx, val in enumerate(np.array([True] * layer.shape[0]).reshape(-1, 1) 
                                                              for layer in self._get_model_layers(bidder_name, exc_layer_type=['output']))
                      })
                      for bidder_name in self.sorted_bidders))
            # big-M variables
            self.L1 = OrderedDict(
                list((bidder_name, 
                      {idx + 1: val for idx, val in enumerate(np.array([self.L] * layer.shape[0], 
                                                                       dtype=np.float32).reshape(-1, 1) 
                                                              for layer in self._get_model_layers(bidder_name, exc_layer_type=['output']))
                      })
                      for bidder_name in self.sorted_bidders))
    ...
    def solve_mip(self,
        ...
        for i in range(0, self.N):
            ...
            logging.info(f"Bidder_{i:>2}: {''.join([str(a) for a in self.x_star[i,:]])}")
    ...
    def __set_active_boolean(self, verbose:bool=False):
        for key in self.sorted_bidders:
            layer = 1
            ...
            for v in range(0, len(Wb), 2):  # loop over layers
                ...
                for r in range(R):
                    ...
                    elif (    0 <  self.upper_bounds_z[key][layer][r][0] <= ts[layer-1] 
                          and 0 <= self.lower_bounds_z[key][layer][r][0] <  ts[layer-1]):
                        self.active[key][layer][r][0] = False
    ...
    def __add_variables_for_hard_parameter_sharing(self, shared_num:int=1, verbose:bool=False):
        # input
        for i in range(len(self.sorted_bidders)):
            self.z.update({(i,0,j): self.Mip.binary_var(name=f"x({i})_{j}") for j in range(self.M)})

        # shared layers
        for layer in range(1, shared_num+1):
            # reference bidder (allow for any index)
            key = self.sorted_bidders[0]

            # shape
            W = self._get_model_weights(key)[2*(layer-1)]
            R, _ = W.shape # R = dim_out

            # output value variables after activation
            self.z.update({(layer,r): self.Mip.continuous_var(name=f"z({layer})_{r}") for r in range(R)})

            # slack variables
            self.eta.update({(layer,r): self.Mip.continuous_var(name=f"eta({layer})_{r}")
                             for r in range(R) if self.active[key][layer][r][0]})

            # binary variables for activation function
            self.y.update({(layer,r): self.Mip.binary_var(name=f"y({layer})_{r}")
                           for r in range(R) if self.active[key][layer][r][0]})
            
            # binary variables for activation function
            self.mu.update({(layer,r): self.Mip.binary_var(name=f"mu({layer})_{r}") 
                            for r in range(R) if self.active[key][layer][r][0]})

        # bidder specific
        for i, key in enumerate(self.sorted_bidders):
            layer = shared_num + 1

            Wb = self._get_model_weights(key)

            for v in range(2*shared_num, len(Wb), 2):  # loop over bidder specific layers
                if verbose:
                    logging.debug('\nLayer: %s', layer)
                ...
                # output value variables after activation
                self.z.update({(i,layer,r): self.Mip.continuous_var(name=f"z({i},{layer})_{r}")
                               for r in range(R)}) 
                
                # binary variables for activation function
                self.y.update({(i,layer,r): self.Mip.binary_var(name=f"y({i},{layer})_{r}")
                               for r in range(R) if self.active[key][layer][r][0]})
                
                # slack variables
                self.eta.update({(i,layer,r): self.Mip.continuous_var(name=f"eta({i},{layer})_{r}")
                                 for r in range(R) if self.active[key][layer][r][0]})
                
                # binary variables for activation function
                self.mu.update({(i,layer,r): self.Mip.binary_var(name=f"mu({i},{layer})_{r}")
                               for r in range(R) if self.active[key][layer][r][0]})
            ...
            R, J = W.shape
            # output value variables after activation
            self.z.update({(i,layer,r): self.Mip.sum(W[r,j] * self.z[(i,layer-1,j)] 
                                                     for j in range(J))
                          for r in range(R)})

    def __add_matrix_constraint_of_shared_layers(self, shared_num:int, verbose:bool=False):
        ref_key = self.sorted_bidders[0]
        if verbose:
            logging.debug('\nAdd Matrix constraints (reference): %s', ref_key)

        layer = 1
        ts = self.Models[ref_key].ts
        Wb = self._get_model_weights(ref_key)

        for v in range(0, 2*shared_num, 2):
            W, b = Wb[v], Wb[v+1]
            R, J = W.shape

            if layer == 1:
                for i, key in enumerate(self.sorted_bidders):
                    z_pre = [self.z[(i, layer-1, j)] for j in range(J)]
                    for r in range(R):
                        # check equality
                        if verbose: logging.debug("Check Equiality")
                        ref_upper_bound_z = self.upper_bounds_z[ref_key][layer][r][0]
                        ref_lower_bound_z = self.lower_bounds_z[ref_key][layer][r][0]
                        ref_L1 = self.L1[ref_key][layer][r]
                        ref_L2 = self.L2[ref_key][layer][r]
                        ref_L3 = self.L3[ref_key][layer][r]
                        ref_L4 = self.L4[ref_key][layer][r]
                        for key in self.sorted_bidders:
                            assert ref_upper_bound_z == self.upper_bounds_z[key][layer][r][0]
                            assert ref_lower_bound_z == self.lower_bounds_z[key][layer][r][0]
                            assert ref_L1 == self.L1[key][layer][r]
                            assert ref_L2 == self.L2[key][layer][r]
                            assert ref_L3 == self.L3[key][layer][r]
                            assert ref_L4 == self.L4[key][layer][r]
                        if verbose:
                            logging.debug('Row: %s', r)
                            logging.debug('W[r,]: %s', W[r, :])
                            logging.debug('b[r]: %s', b[r])
                            logging.debug('upper (preactivated) z-bound: {}, {}, {}, {}'.format(key, layer, r, self.upper_bounds_z[key][layer][r][0]))
                            logging.debug('lower (preactivated) z-bound: {}, {}, {}, {}'.format(key, layer, r, self.lower_bounds_z[key][layer][r][0]))
                        ...
                        if (self.upper_bounds_z[key][layer][r][0] <= 0):
                            if verbose:
                                logging.debug(
                                    'Case 1: upper (preactivated) z-bound: {}, {} is <=0 => add z==0 constraints'.format(layer, r)
                                )
                            self.Mip.add_constraint(ct=self.z[(layer, r)] == 0,
                                                    ctname="Case1_CT_Shared_Layer{}_Row{}".format(layer, r))
                            self.removed_activation_cts += 1
                        # Case 2: "for sure in the linear range of the UNN-activation function" (given IA Bounds on Neurons)
                        elif (      0 <  self.upper_bounds_z[ref_key][layer][r][0] <= ts[layer-1] 
                                and 0 <= self.lower_bounds_z[ref_key][layer][r][0] <  ts[layer-1]):
                            if verbose:
                                logging.debug(
                                    'Case 2: upper (preactivated) z-bound is in (0,t] AND lower (preactivated) z-bound is in [0,t): {}, {} => add z==Wz_pre + b constraints'.format(layer, r)
                                )
                            self.Mip.add_constraint(
                                ct=(self.Mip.sum(W[r, j] * z_pre[j] for j in range(J)) + b[r] == self.z[(layer, r)]), 
                                ctname="Case2_CT_Shared_Layer{}_Row{}".format(layer, r)
                            )
                            self.removed_activation_cts += 1
                        ...
                        # Case 4: "non-decidable"
                            ...
                            # (i.)
                            self.Mip.add_constraint(
                                ct=(self.Mip.sum(W[r, j] * z_pre[j] for j in range(J)) + b[r] <= self.eta[(layer, r)]),
                                ctname="AffineCT_Shared_Layer{}_Row{}_eta_i_lb".format(layer, r)
                            )
                            self.Mip.add_constraint(
                                ct=(self.eta[(layer, r)] <= self.Mip.sum(W[r, j] * z_pre[j] for j in range(J)) + b[r] + self.y[(layer, r)] * self.L1[ref_key][layer][r][0]),
                                ctname="AffineCT_Shared_Layer{}_Row{}_eta_i_ub".format(layer, r)
                            )

                            # (ii.)
                            self.Mip.add_constraint(
                                ct=(0 <= self.eta[(layer, r)]),
                                ctname="AffineCT_Shared_Layer{}_Row{}_eta_ii_lb".format(layer, r)
                            )
                            self.Mip.add_constraint(
                                ct=(self.eta[(layer, r)] <= (1 - self.y[(layer, r)]) * self.L2[ref_key][layer][r][0]),
                                ctname="AffineCT_Shared_Layer{}_Row{}_eta_ii_ub".format(layer, r)
                            )

                            # (iii.)
                            self.Mip.add_constraint(
                                ct=self.eta[(layer, r)] - self.mu[(layer, r)] * self.L3[ref_key][layer][r][0] <= self.z[(layer, r)],
                                ctname="BinaryCT_Shared_Layer{}_Row{}_z_iii_lb".format(i, layer, r)
                            )
                            self.Mip.add_constraint(
                                ct=self.z[(layer, r)] <= self.eta[(layer, r)],
                                ctname="BinaryCT_Shared_Layer{}_Row{}_z_iii_up".format(i, layer, r)
                            )

                            # (iv.)
                            self.Mip.add_constraint(
                                ct=ts[layer-1] - (1-self.mu[(layer, r)]) * self.L4[ref_key][layer][r][0] <= self.z[(layer, r)],
                                ctname="BinaryCT_Shared_Layer{}_Row{}_z_iv_lb".format(layer, r)
                            )
                            self.Mip.add_constraint(
                                ct=self.z[(layer, r)] <= ts[layer-1],
                                ctname="BinaryCT_Shared_Layer{}_Row{}_z_iv_up".format(layer, r)
                            )
            else:
                z_pre = [self.z[(layer-1, j)] for j in range(J)]
                for r in range(R):
                    # check equality
                    if verbose:
                        logging.debug("Check Equiality")
                    ref_upper_bound_z = self.upper_bounds_z[ref_key][layer][r][0]
                    ref_lower_bound_z = self.lower_bounds_z[ref_key][layer][r][0]
                    ref_L1 = self.L1[ref_key][layer][r]
                    ref_L2 = self.L2[ref_key][layer][r]
                    ...
                    if verbose:
                        ...
                        logging.debug('W[r,]: %s', W[r, :])
                        logging.debug('b[r]: %s', b[r])
                        logging.debug('upper (preactivated) z-bound: {}, {}, {}, {}'.format(ref_key, layer, r, self.upper_bounds_z[ref_key][layer][r][0]))
                        logging.debug('lower (preactivated) z-bound: {}, {}, {}, {}'.format(ref_key, layer, r, self.lower_bounds_z[ref_key][layer][r][0]))
                        logging.debug('L1: {}, {}, {}, {}'.format(ref_key, layer, r, self.L1[ref_key][layer][r]))
                        logging.debug('L2: {}, {}, {}, {}'.format(ref_key, layer, r, self.L2[ref_key][layer][r]))
                        logging.debug('L3: {}, {}, {}, {}'.format(ref_key, layer, r, self.L3[ref_key][layer][r]))
                        logging.debug('L4: {}, {}, {}, {}'.format(ref_key, layer, r, self.L4[ref_key][layer][r]))

                    # Case 1 "for sure in the zero range of the UNN-activation function" (given IA Bounds on Neurons)
                    if (self.upper_bounds_z[ref_key][layer][r][0] <= 0):
                        if verbose:
                            logging.debug(
                                'Case 1: upper (preactivated) z-bound: {}, {} is <=0 => add z==0 constraints'.format(layer, r)
                            )
    ...
    def __add_matrix_constraint_of_bidder_specific_models(self, shared_num:int, i:int, verbose:bool=False):
        ...
        for v in range(2*(layer-1), len(Wb), 2):  # loop over layers
            W, b = Wb[v], Wb[v+1]
            R, J = W.shape

            if layer == shared_num + 1:
                z_pre = [self.z[(layer-1, j)] for j in range(J)]
            else:
                z_pre = [self.z[(i, layer-1, j)] for j in range(J)]
            # add constraints
            for r in range(R):
                ...
                if (self.upper_bounds_z[key][layer][r][0] <= 0):
                    if verbose:
                        logging.debug(
                            'Case 1: upper (preactivated) z-bound: {}, {}, {} is <=0 => add z==0 constraints'.format(key, layer, r)
                        )
                ...
                # Case 2: "for sure in the linear range of the UNN-activation function" (given IA Bounds on Neurons)
                    ...
                    if verbose:
                        logging.debug(
                            'Case 2: upper (preactivated) z-bound is in (0,t] AND lower (preactivated) z-bound is in [0,t): {}, {}, {} => add z==Wz_pre + b constraints'.format(key, layer, r)
                        )
                    self.Mip.add_constraint(
                        ct=(self.Mip.sum(W[r, j] * z_pre[j] for j in range(J)) + b[r] == self.z[(i, layer, r)]), 
                        ctname="Case2_CT_Bidder{}_Layer{}_Row{}".format(i, layer, r)
                    )
                    self.removed_activation_cts += 1
                ...
                # Case 4: "non-decidable"
                    ...
                    # (i.)
                    self.Mip.add_constraint(
                        ct=(self.Mip.sum(W[r, j] * z_pre[j] for j in range(J)) + b[r] <= self.eta[(i, layer, r)]),
                        ctname="AffineCT_Bidder{}_Layer{}_Row{}_eta_i_lb".format(i, layer, r)
                    )
                    self.Mip.add_constraint(
                        ct=(self.eta[(i, layer, r)] <= self.Mip.sum(W[r, j] * z_pre[j] for j in range(J)) + b[r] + self.y[(i, layer, r)] * self.L1[key][layer][r][0]),
                    ...
                    self.Mip.add_constraint(
                        ct=(self.eta[(i, layer, r)] <= (1 - self.y[(i, layer, r)]) * self.L2[key][layer][r][0]),
                        ctname="AffineCT_Bidder{}_Layer{}_Row{}_eta_ii_ub".format(i, layer, r)
                    )

                    # (iii.)
                    self.Mip.add_constraint(
                        ct=self.eta[(i, layer, r)] - self.mu[(i, layer, r)] * self.L3[key][layer][r][0] <= self.z[(i, layer, r)],
                    ...
                    # (iv.)
                    self.Mip.add_constraint(
                        ct=ts[layer-1] - (1-self.mu[(i, layer, r)]) * self.L4[key][layer][r][0] <= self.z[(i, layer, r)],
    ...
    def _add_matrix_contraints_for_hard_parameter_sharing(self, shared_num:int, verbose:bool=False):
        self.__set_active_boolean(verbose=verbose)
        self.__add_variables_for_hard_parameter_sharing(shared_num=shared_num, verbose=verbose)                                    

        self.__add_matrix_constraint_of_shared_layers(shared_num=shared_num, verbose=verbose)
        for i in range(len(self.sorted_bidders)):
            self.__add_matrix_constraint_of_bidder_specific_models(i=i, shared_num=shared_num, verbose=verbose)
    ...
    def initialize_mip(self,
        ...
        if False:
            self._add_matrix_contraints_for_hard_parameter_sharing(shared_num=shared_num, verbose=verbose)
        else:
        ...
        objective = self.Mip.sum(
            self.Models[self.sorted_bidders[i]]._target_max * self.z[(i, self.Models[self.sorted_bidders[i]]._num_hidden_layers + 1, 0)] 
            for i in range(0, self.N)
        )
...
# %%
logging.info('Class NN_MIP_TORCH imported')
